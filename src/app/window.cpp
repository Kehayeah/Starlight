...
Starlight::Starlight(Arbiter &arbiter)
    : QWidget()
    , arbiter(arbiter)
    , rail()
    , body()
...
void Starlight::init()
{
    ...
    void Starlight::init_connected_pages()
    {
...
void Starlight::set_page(Page *page)
{
...
QWidget *Starlight::control_bar() const
{
...
QWidget *Starlight::power_control() const
{
    auto widget = new QWidget();
    auto layout = new QHBoxLayout(widget);
    layout->setContentsMargins(0, 0, 0, 0);
    ...
    , arbiter(this)
{
    ...
    auto starlight = new Starlight(this->arbiter);
    stack->addWidget(starlight);
    starlight->init();
